...
public fun TestAbstract_process_wrapper(env: napi_env?, info: napi_callback_info?): napi_value? =
    ...
    memScoped {
        ...
        val instance = thisVar.value!!.unwrapKotlinObject<TestAbstract<Any?>>(env!!)!!
    ...
    println("Error in TestAbstract_process_wrapper: ${e.message}")
    e.printStackTrace()
    napi.napi_throw_error(env, null, e.message ?: "Unknown Kotlin exception")
    null
}

public fun TestAbstract_sayHello_wrapper(env: napi_env?, info: napi_callback_info?): napi_value? =
    try {
    memScoped {
        val argc = alloc<size_tVar>()
        argc.value = 0u
    ...
    println("Error in TestAbstract_sayHello_wrapper: ${e.message}")
    e.printStackTrace()
    napi.napi_throw_error(env, null, e.message ?: "Unknown Kotlin exception")
    null
}
